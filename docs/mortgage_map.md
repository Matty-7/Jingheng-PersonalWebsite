...
`lib/mortgage_state.ts` owns view, reader, history, hierarchy filters, paths and comparison selection through semantic reducer actions. History traversal moves the existing cursor without recording another visit; a new selection after Back replaces only the forward branch. `MortgageMap` coordinates domain actions and search. `useMortgageCamera` owns measurement, camera layout, pan/pinch/keyboard controls and recentering; `useMortgageReader` owns URL hydration/sync, clipboard feedback and reader focus/scroll. React hook exports retain the required `useX` naming convention. Canvas, Reader, Paths, Comparison and List have separate typed rendering components. `useAtlasExpansion` continues to own modal containment and focus restoration. A valid concept URL opens the concept title in Connections.
...
`mortgage_map.css` is the ordered entry point for seven sections in `styles/`: foundation/paths, navigation, canvas, reader/lists, responsive rules, expanded canvas and comparisons. The normal caption and navigation dock now have one canonical layout in `canvas.css`; obsolete absolute positions, mobile offsets and duplicate reader rules were removed. `expanded_canvas.css` contains expansion controls and layout. Keep the entry order: these styles do not introduce cascade layers. Global styling is owned by the ordered semantic files under `app/styles/`; 46 shadowed declarations and 19 obsolete book-detail/postscript rules were removed without moving competing selectors.
