...
Google Maps embeds the selected place through the shared free Maps Embed API helper and opens the same query through keyless Maps URLs. Literary, Music and Film Map use one runtime-configured browser key. There is no Maps JavaScript API, Places data access or metered fallback. Missing configuration omits the iframe while preserving the external map link and location description. The application cannot reliably inspect cross-origin iframe content and does not declare map success from an iframe load event. See [the Google Maps cost policy](google_maps.md).
