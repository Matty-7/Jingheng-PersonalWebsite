...
function AlbumCover({
  ...
  if (failed)
    return (
      <span className="sound-cover-fallback">
        <Disc3 aria-hidden="true" />
        <span>{small ? track.artist : 'Album artwork unavailable'}</span>
      </span>
    );
  return (
    <Image
      src={track.artwork_url}
      alt={small ? '' : `${track.album} album cover`}
      width={small ? 100 : 240}
      height={small ? 100 : 240}
      unoptimized
...
function GoogleMusicMap({
  ...
  return (
    <section
      className="sound-map-panel"
      aria-label={overview ? 'Map of all songs' : `Map of ${place.name}`}
    >
      <div className="sound-map-heading">
        <span>
          <MapPin size={16} aria-hidden="true" />
          {overview ? 'All song locations' : place.name}
        </span>
        <span>
          {overview
            ? `${music_places.length} places · ${music_tracks.length} songs`
            : 'Selected place'}
        </span>
      </div>
      <div className="sound-map-frame">
        {overview ? (
          ...
          <p className="sound-location-note">
            The map is unavailable. Use the Google Maps link below.
          </p>
        )}
      </div>
      <div className="sound-map-footer">
        <div>
          <span className="sound-label">{place.precision}</span>
          <p>
            {place.name} · {place.area}
          </p>
        </div>
        <a
          href={google_music_url(place)}
          target="_blank"
...
export function NycMusicMap({ google_maps_key }: { google_maps_key: string }) {
  ...
  function select_track(next_track: MusicTrack | undefined) {
    ...
    update({
      ...state,
      track_id: next_track?.id ?? null,
      place_id: next_track?.place_ids[0] ?? '',
    });
  }
  ...
  function change_query(
    next_query: string,
    history_mode: 'push' | 'replace' = 'replace',
  ) {
    if (overview && !search_music(next_query).length) stop_preview();
    update({ ...state, query: next_query }, history_mode);
  }
  ...
  function move_shelf(direction: number) {
    ...
    shelf.current?.scrollBy({
      left: direction * (shelf.current.clientWidth * 0.75),
      behavior: reduced ? 'instant' : 'smooth',
    });
  }

  return (
    <div className="sound-explorer">
      <MusicPlaylist />
      <div className="sound-catalog-heading">
        <div>
          <Disc3 size={18} aria-hidden="true" />
          <span>
            {music_tracks.length} songs · {music_places.length} places
          </span>
        </div>
        <label className="sound-search">
          ...
          <input
            ...
            placeholder="Song, artist or place"
            aria-label="Search songs, artists or places"
          />
          {query && (
            <button
              aria-label="Clear search"
              onClick={() => change_query('', 'push')}
            >
              <X size={16} aria-hidden="true" />
            </button>
          )}
        </label>
        <div className="sound-shelf-controls">
          <button
            disabled={album_scroll.reduced}
            aria-label={
              album_scroll.reduced
                ? 'Album scrolling off: reduced motion'
                : album_scroll.scrolling
                  ? 'Pause album scrolling'
                  : 'Resume album scrolling'
            }
            title={
              album_scroll.reduced
          ...
          <button aria-label="Previous songs" onClick={() => move_shelf(-1)}>
            <ChevronLeft size={19} aria-hidden="true" />
          </button>
          <button aria-label="More songs" onClick={() => move_shelf(1)}>
            <ChevronRight size={19} aria-hidden="true" />
          </button>
        </div>
      </div>
      <output className="sound-search-status">
        {query
          ? `${results.length} matching ${results.length === 1 ? 'song' : 'songs'}`
          : 'Choose a record to find its places.'}
      </output>
      <div
        className={`sound-shelf${album_scroll.scrolling ? ' is-scrolling' : ''}`}
        ref={shelf}
        aria-label="Song collection"
      >
        {results.map((item) => (
          <button
            disabled={!ready}
          ...
          >
            <span className="sound-album-art">
              ...
              <span className="sound-album-number">
                {track_id === item.id ? (
                  <Check size={14} aria-hidden="true" />
                ) : (
                  String(music_tracks.indexOf(item) + 1).padStart(2, '0')
                )}
              </span>
            </span>
      ...
      <fieldset className="sound-map-controls" aria-label="Map view">
        <button aria-pressed={overview} onClick={show_all_places}>
          <MapPin size={16} aria-hidden="true" />
          Show all places
        </button>
        <button
          aria-pressed={!overview}
          onClick={() => update({ ...state, overview: false })}
        >
          Selected place
        </button>
        {overview && (
          <span>
            All songs, including those outside your search. Zoom in to separate
            nearby places.
          </span>
        )}
      </fieldset>
      {track && place ? (
            ...
            >
              <div className="sound-record-top" key={`cover-${track.id}`}>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={`Open ${track.album} on Apple Music`}
                >
                  <AlbumCover key={track.id} track={track} />
                </a>
                <div>
                  <p className="sound-label">
                    {track.year ?? 'Year unverified'} · {track.genre}
                  </p>
                  <h2>{track.title}</h2>
                  <p className="sound-artist">{track.artist}</p>
                  <p className="sound-album-name">{track.album}</p>
                </div>
              </div>
              ...
              <output className="sound-preview-message">
                {message || 'Song preview provided courtesy of iTunes.'}
              </output>
              <div className="sound-store-links">
                <a
                  href={track.apple_music_url}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={`Download ${track.title} on iTunes`}
                >
                  <Image
                    src="/images/music-map/itunes-badge.svg"
              ...
              <div className="sound-story" key={track.id}>
                ...
                {track.excerpt && (
                  <span className="sound-excerpt-note">
        ...
        <div className="sound-empty">
          ...
          <p>Try a different song, artist or New York place.</p>
          <button onClick={() => change_query('', 'push')}>
            Show all songs
          </button>
        </div>
      )}
