...
function GoogleMusicMap({ place, overview, fit_request, focus_request, on_select }: { place: MusicPlace; overview: boolean; fit_request: number; focus_request: MusicFocus | null; on_select: (track: MusicTrack, place_id: string) => void }) {
  return <section className="sound-map-panel" aria-label={overview ? 'Map of all songs' : `Map of ${place.name}`}>
    ...
    <div className="sound-map-frame">
      {overview ? <MusicOverview selected_place_id={place.id} fit_request={fit_request} focus_request={focus_request} on_select={on_select} /> : <iframe title={`Google Maps: ${place.name}`} src={google_music_embed_url(place)} referrerPolicy="no-referrer-when-downgrade" allowFullScreen />}
...
export function NycMusicMap() {
  ...
  return <div className="sound-explorer">
    ...
    <div className={`sound-shelf${album_scroll.scrolling ? ' is-scrolling' : ''}`} ref={shelf} aria-label="Song collection">
      {results.map((item) => <button key={item.id} data-track-id={item.id} className="sound-album" aria-pressed={track_id === item.id} aria-label={`Select ${item.title} by ${item.artist}`} onClick={() => { select_track(item); set_focus_request({ place_id: item.place_ids[0] }); }}><span className="sound-album-art"><AlbumCover track={item} small /><span className="sound-album-number">{track_id === item.id ? <Check size={14} aria-hidden="true" /> : String(music_tracks.indexOf(item) + 1).padStart(2, '0')}</span></span><strong>{item.title}</strong><span>{item.artist}</span><span className="sound-album-place">{track_places(item)[0].name}</span></button>)}
    </div>
    ...
    {track && place ? <>
      <div className="sound-place-picker"><span className="sound-label">Places in this song</span><div>{places.map((item) => <button key={item.id} aria-pressed={place.id === item.id} onClick={() => { set_place_id(item.id); set_focus_request({ place_id: item.id }); }}><MapPin size={15} aria-hidden="true" />{item.name}</button>)}</div></div>
      <div className="sound-workspace">
        <GoogleMusicMap place={place} overview={overview} fit_request={fit_request} focus_request={focus_request} on_select={select_map_track} />
        <article className={`sound-record${playing ? ' is-playing' : ''}`} aria-label="Selected song">
          ...
          <div className="sound-story" key={track.id}><p className="sound-label">{track.excerpt ? 'In the lyrics' : 'In the title'}</p>{track.excerpt ? <blockquote>“{track.excerpt}”</blockquote> : <h3>{place.name}</h3>}<p>{track.note}</p><a className="sound-source" href={track.source_url} target="_blank" rel="noopener noreferrer">{track.source_label}<ArrowUpRight size={14} aria-hidden="true" /></a>{track.excerpt && <span className="sound-excerpt-note">Short excerpt. Full lyrics at the source.</span>}</div>
        </article>
      </div>
