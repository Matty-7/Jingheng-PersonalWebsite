...
import { literary_embed_url, literary_entries, literary_maps_url, literary_works, literary_place_count, search_literary_entries, type LiteraryWork } from '@/lib/nyc_literary_map';
...
function LiteraryCover({ work, small = false }: { work: LiteraryWork; small?: boolean }) {
  ...
  return <Image unoptimized src={work.cover.src} alt={small ? '' : work.cover.alt} width={240} height={360} loading="lazy" onError={() => set_failed(true)} />;
}
...
export function NycLiteraryMap({ google_maps_key }: { google_maps_key: string }) {
  const ready = useSyncExternalStore(subscribe_to_readiness, client_ready, server_ready);
  ...
  const embed_url = selected_entry ? literary_embed_url(selected_entry, google_maps_key) : null;
  ...
  function update_filters(next_work: string, next_query: string) {
    const matches = search_literary_entries(next_work, next_query);
    const current = matches.find((entry) => entry.id === selected_entry?.id) ?? matches[0];
    set_work_id(next_work);
    set_query(next_query);
    set_selected_id(current?.id ?? '');
  }
  ...
  function clear_filters() {
    set_work_id('all');
    set_query('');
    set_selected_id(literary_entries[0].id);
  }
  ...
  return <>
    <section className="literary-collection" aria-label="Choose a work">
      <div className="literary-collection-label"><span>THE READING LIST</span><span>{literary_works.length} works · {literary_entries.length} passages · {literary_place_count} places</span></div>
      <div className="literary-search-row"><label htmlFor="literary-search">Search this collection<input disabled={!ready} id="literary-search" type="search" value={query} onChange={(event) => update_filters(work_id, event.target.value)} placeholder="Writer, book or place" /></label><div><output>{visible_entries.length} matching passages</output>{(query || work_id !== 'all') && <button disabled={!ready} onClick={clear_filters}>Clear filters</button>}</div></div>
      <details className="literary-catalog"><summary>Browse {literary_works.length} works <span>{work_id === 'all' ? 'All works' : literary_works.find((work) => work.id === work_id)?.title}</span></summary>
      <div className="literary-work-buttons">
        <button disabled={!ready} className="literary-all" aria-pressed={work_id === 'all'} onClick={() => choose_work('all')}><BookOpen size={24} aria-hidden="true" /><strong>All works</strong><span>Books &amp; magazines</span>{work_id === 'all' && <Check size={16} aria-hidden="true" />}</button>
        {literary_works.map((work) => <button disabled={!ready} key={work.id} aria-pressed={work_id === work.id} onClick={() => choose_work(work.id)}>
    ...
    {!selected_entry || !selected_work ? <section className="literary-empty" aria-label="No matching passages"><h2>No passages found</h2><p>Try another writer, book or place, or clear your filters.</p><button disabled={!ready} onClick={clear_filters}>Clear filters</button></section> : <div className="literary-workspace">
      <section className="literary-map-panel" ref={map_panel} aria-label="Google map of the selected place" tabIndex={-1}>
        ...
        {map_visible && embed_url ? <iframe key={`${selected_entry.id}-${map_reload}`} className="literary-google-map" title={`Google Maps: ${selected_entry.place}`} src={embed_url} referrerPolicy="strict-origin-when-cross-origin" allowFullScreen /> : <div className="literary-map-hidden"><MapPin size={26} aria-hidden="true" /><p>{selected_entry.place}</p><span>{selected_entry.area}</span>{embed_url ? <button disabled={!ready} onClick={() => set_map_visible(true)}>Show Google map</button> : <p>The map is unavailable. Use the Google Maps link below.</p>}</div>}
        <div className="literary-map-help"><a href={literary_maps_url(selected_entry)} target="_blank" rel="noopener noreferrer">Open in Google Maps <ArrowUpRight size={14} aria-hidden="true" /></a>{embed_url && <button disabled={!ready} onClick={() => set_map_visible(!map_visible)}>{map_visible ? 'Hide map' : 'Show map'}</button>}{map_visible && embed_url && <button disabled={!ready} onClick={() => set_map_reload((value) => value + 1)}>Reload map</button>}</div>
        <p className="literary-map-note">Showing one selected place. If the map is unavailable, use the Google Maps link.</p>
        <details className="literary-places"><summary>Browse {visible_entries.length} passages <span>{selected_entry.place}</span></summary>
        <nav className="literary-place-index" aria-label="Literary places">
          <p className="literary-index-label">{visible_entries.length} PASSAGES IN THIS SELECTION</p>
          <ol>{visible_entries.map((entry, index) => <li key={entry.id}><button disabled={!ready} aria-pressed={entry.id === selected_entry.id} onClick={() => show_passage(entry.id)}><span>{String(index + 1).padStart(2, '0')}</span><span><strong>{entry.place}</strong><small>{literary_works.find((work) => work.id === entry.work_id)!.title} · {literary_works.find((work) => work.id === entry.work_id)!.author}</small></span>{entry.id === selected_entry.id ? <BookOpen size={18} aria-hidden="true" /> : <span aria-hidden="true">↗</span>}</button></li>)}</ol>
        </nav>
        </details>
      </section>
      <article className="literary-reader" ref={reading_panel} tabIndex={-1} aria-label={`Reading: ${selected_entry.place}`}>
        <div className="literary-reader-top"><span>PASSAGE {String(selected_index + 1).padStart(2, '0')} / {String(visible_entries.length).padStart(2, '0')}</span><div><button aria-label="Previous passage" disabled={!ready || selected_index === 0} onClick={() => show_passage(visible_entries[selected_index - 1].id)}><ChevronLeft size={19} /></button><button aria-label="Next passage" disabled={!ready || selected_index === visible_entries.length - 1} onClick={() => show_passage(visible_entries[selected_index + 1].id)}><ChevronRight size={19} /></button></div></div>
        <div key={selected_entry.id} className="literary-reading-content">
          ...
          <div className="literary-reader-actions"><a href={literary_maps_url(selected_entry)} target="_blank" rel="noopener noreferrer">Open in Google Maps <ArrowUpRight size={17} aria-hidden="true" /></a><button disabled={!ready} onClick={() => { map_panel.current?.focus({ preventScroll: true }); map_panel.current?.scrollIntoView({ behavior: 'instant', block: 'start' }); }}>View map</button></div>
